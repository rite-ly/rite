...
///
/// [`KeyAlgorithm`](rite_sdk::KeyAlgorithm) is `#[non_exhaustive]` and defined
/// elsewhere, so the wildcard arm is mandatory and a new variant reads as
/// unlimited until it is listed here. The refusal in
/// `OpenSslBackend::generate_key` is what catches it instead.
#[must_use]
pub fn build_limitation(algorithm: rite_sdk::KeyAlgorithm) -> Option<&'static str> {
    use rite_sdk::KeyAlgorithm;
    // Everything outside the post-quantum set is generated by every build, so
    // no build is what limits it.
