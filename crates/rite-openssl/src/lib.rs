...
pub fn build_limitation(algorithm: rite_sdk::KeyAlgorithm) -> Option<&'static str> {
    ...
    // the question. Everything outside the post-quantum set is either
    // generated by every build or by none: no backend generates a symmetric
    // key yet, and that holds everywhere, so it is not this function's to
    // report.
