...
fn apply_concat(args: &[Expression], ctx: &HandlerContext) -> Result<Value, ExecutionError> {
    ...
    // Sized up front. A buffer that grows as it goes leaves each outgrown
    // allocation behind unwiped, with a prefix of the content in it.
    let mut result = Vec::with_capacity(parts.iter().map(|part| part.len()).sum());
    for part in parts {
