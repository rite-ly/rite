...
It is usually the fingerprint the origin ceremony's `generate_keypair` step
recorded.
...
|---|---|---|
| `CMS-AES-256-GCM` | CMS `AuthEnvelopedData` DER | the blob is archived, or the recipient reads CMS. The default. |
...
Both raw schemes need an RSA recipient. The wrapped key is always PKCS#8
