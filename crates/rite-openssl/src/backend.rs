...
/// `len` bytes from OpenSSL's random generator, wiped when dropped.
pub(crate) fn random_bytes(len: usize) -> Result<Zeroizing<Vec<u8>>, BackendError> {
    let mut bytes = Zeroizing::new(vec![0u8; len]);
    openssl::rand::rand_bytes(&mut bytes).map_err(|e| ossl_err("Generate random bytes", &e))?;
    Ok(bytes)
}
...
fn generate_ml_kem(algorithm: KeyAlgorithm) -> Result<PKey<Private>, BackendError> {
    ...
    let seed = random_bytes(ML_KEM_SEED_LEN)?;
...
fn generate_ml_dsa(algorithm: KeyAlgorithm) -> Result<PKey<Private>, BackendError> {
    ...
    let seed = random_bytes(ML_DSA_SEED_LEN)?;
    PKey::private_key_from_seed(None, key_type, None, &seed)
...
impl KeyStoreBackend for OpenSslBackend {
    fn generate_key(&mut self, spec: KeySpec) -> Result<KeyMetadata, BackendError> {
        ...
        if let Some(length) = spec.algorithm.key_bytes() {
            let secret = random_bytes(length)?;
